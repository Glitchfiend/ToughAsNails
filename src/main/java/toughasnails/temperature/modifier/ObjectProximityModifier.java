...
public class ObjectProximityModifier extends TemperatureModifier {
		...
		for (int x = -3; x <= 3; x++) {
			for (int y = -2; y <= 2; y++) {
				for (int z = -3; z <= 3; z++) {
					...
					if (getBlockTemperature(state) != 0.0F)
						tempSourceBlocks++;
				}
			}
		}
		...
		for (int x = -3; x <= 3; x++) {
			for (int y = -2; y <= 2; y++) {
				for (int z = -3; z <= 3; z++) {
					...
					blockTemperatureModifier += getBlockTemperature(state);
				}
			}
		}
	...
	public static float getBlockTemperature(IBlockState state) {
		...
		if (TANConfig.blockTemperatureData.containsKey(blockName)) {
			ArrayList<BlockTemperatureData> blockTempData = TANConfig.blockTemperatureData
					.get(blockName);
			...
			for (BlockTemperatureData tempData : blockTempData) {
				...
				for (String comparisonProperty : tempData.useProperties) {
					IProperty<?> targetProperty = BlockStateUtils
							.getPropertyByName(state, comparisonProperty);

					if (!(state.getValue(targetProperty) == tempData.state
		...
		// Handle materials, but only if we didn't already find an actual block
		// to use:
	...
	public Temperature modifyTarget(World world, BlockPos position,
		...
		for (int x = -3; x <= 3; x++) {
			for (int y = -2; y <= 2; y++) {
				for (int z = -3; z <= 3; z++) {
					BlockPos pos = position.add(x, y - 1, z);
					IBlockState state = world.getBlockState(pos);
