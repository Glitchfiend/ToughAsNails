...
public class SeasonASMHelper {
	...
	public static boolean canSnowAtInSeason(World world, BlockPos pos,
			boolean checkLight, Season season) {
		...
		if (!SeasonHelper.canSnowAtTempInSeason(season, temperature)) {
			return false;
		} else if (biome == Biomes.RIVER || biome == Biomes.OCEAN
	...
	public static boolean canBlockFreezeInSeason(World world, BlockPos pos,
			boolean noWaterAdj, Season season) {
		Biome Biome = world.getBiomeGenForCoords(pos);
		float temperature = Biome.getFloatTemperature(pos);
		...
		if (!SeasonHelper.canSnowAtTempInSeason(season, temperature)) {
			...
			if (pos.getY() >= 0 && pos.getY() < 256
				...
				if ((block == Blocks.WATER || block == Blocks.FLOWING_WATER)
					...
					boolean flag = world.isWater(pos.west())
	...
	public static boolean isRainingAtInSeason(World world, BlockPos pos,
			Season season) {
		Biome biome = world.getBiomeGenForCoords(pos);
	...
	public static float getFloatTemperature(Biome biome, BlockPos pos) {
		Season season = new SeasonTime(SeasonHandler.clientSeasonCycleTicks)
				.getSubSeason().getSeason();
	...
	public static void onUpdateTick(Block block, World world, BlockPos pos) {
		...
		if (!temperatureWithering) {
			...
			if (season == Season.WINTER
				...
				if (block instanceof IDecayableCrop
					...
					world.setBlockState(pos,
							TANBlocks.dead_crops.getDefaultState());
				} else if (GameplayConfigurationHandler.EXTERNAL_DECAYING_CROPS
			...
			if (targetTemperature > maxLiving
