...
public class ItemPiles extends Item
{
    
    public enum PileType implements IStringSerializable
    {
        DIRT, GRAVEL, SAND, RED_SAND;
    ...
    {
        for (PileType contents : PileType.values())
        {
    ...
    public PileType getContentsType(ItemStack stack)
    {
        int meta = stack.getMetadata();
        try {
            return PileType.values()[meta];
        } catch (Exception e) {
            // if metadata is out of bounds return honey as a default (should never happen)
            return PileType.DIRT;
        }
    }
    ...
    {
        return super.getUnlocalizedName() + "_of_" + this.getContentsType(stack).getName();
    }  
}
